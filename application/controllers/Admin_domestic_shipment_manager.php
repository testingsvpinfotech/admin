...
// print_r($rate);die; 
	...
	public function view_domestic_topay_shipment($offset = 0, $searching = '')
	{
		//print_r($this->session->all_userdata());
		if ($this->session->userdata('userId') == '') {
			redirect('admin');
			...
			$user_id = $this->session->userdata("userId");
			$data['customer'] = $this->basic_operation_m->get_query_result_array('SELECT * FROM tbl_customers WHERE customer_type IN (1,2) ORDER BY customer_name ASC');
			...
			if ($all_data) {
				...
				foreach ($all_data as $ke => $vall) {
					if ($ke == 'filter' && !empty($vall)) {
						...
						$filterCond .= " AND tbl_domestic_invoice.franchise_id = '$vall'";
					} elseif ($ke == 'from_date' && !empty($vall)) {
						$filterCond .= " AND tbl_domestic_invoice.invoice_date >= '$vall'";
					} elseif ($ke == 'to_date' && !empty($vall)) {
						$filterCond .= " AND tbl_domestic_invoice.invoice_date <= '$vall'";
					}
			...
			if (
				...
				$resActt = $this->db->query("SELECT * FROM tbl_domestic_invoice JOIN tbl_domestic_invoice_detail ON tbl_domestic_invoice_detail.invoice_id = tbl_domestic_invoice.id
				JOIN tbl_domestic_booking ON tbl_domestic_booking.pod_no = tbl_domestic_invoice_detail.pod_no WHERE 1 $filterCond ");
				$resAct = $this->db->query("SELECT *,tbl_domestic_invoice.invoice_no as invoice_no1 FROM tbl_domestic_invoice 
				JOIN tbl_domestic_invoice_detail ON tbl_domestic_invoice_detail.invoice_id = tbl_domestic_invoice.id
				JOIN tbl_domestic_booking ON tbl_domestic_booking.pod_no = tbl_domestic_invoice_detail.pod_no
				LEFT JOIN tbl_domestic_topay_invoice_bill ON tbl_domestic_topay_invoice_bill.invoice_id = tbl_domestic_invoice.id
				WHERE tbl_domestic_invoice.payment_type = 'TOPAY' $filterCond ORDER BY tbl_domestic_invoice.id DESC limit " . $offset . ",50");
                // echo $this->db->last_query();die;
				$this->load->library('pagination');
			...
			$data['courier_company'] = $this->basic_operation_m->get_all_result("courier_company", $whr_c);
			$data['mode_details'] = $this->basic_operation_m->get_all_result("transfer_mode", '');
			$this->load->view('admin/domestic_shipment/view_domestic_topay_unbill_shipment', $data);
		}

	}
	public function view_topay_billing($offset = 0, $searching = '')
	{
		//print_r($this->session->all_userdata());
		if ($this->session->userdata('userId') == '') {
			redirect('admin');
			...
			$data['courier_company'] = $this->basic_operation_m->get_all_result("courier_company", $whr_c);
			$data['mode_details'] = $this->basic_operation_m->get_all_result("transfer_mode", '');
			$this->load->view('admin/payment_module/view_topay_billing', $data);
		}

	}

	public function getRecitNO(){
		$data = [];
		$pod_no = $this->input->post('pod_no');
		$invoice = $this->db->query("SELECT tbl_domestic_invoice.* FROM tbl_domestic_invoice JOIN tbl_domestic_invoice_detail ON tbl_domestic_invoice_detail.invoice_id = tbl_domestic_invoice.id  WHERE tbl_domestic_invoice_detail.pod_no ='$pod_no'")->row();
        $result = $this->db->query("SELECT MAX(id) AS id FROM tbl_domestic_topay_invoice_bill")->row();
		$id = $result->id + 1;
		$data['ResciptNO'] = $ResciptNO = 110000000 + $id;
	...
	public function TopayInvoicepaid(){
         if(!empty($_POST)){
			$pod_no = $this->input->post('pod_no');
			$invoice = $this->db->query("SELECT tbl_domestic_invoice.* FROM tbl_domestic_invoice JOIN tbl_domestic_invoice_detail ON tbl_domestic_invoice_detail.invoice_id = tbl_domestic_invoice.id  WHERE tbl_domestic_invoice_detail.pod_no ='$pod_no'")->row();
			$data = [
				...
				'ResciptNO'=>$this->input->post('ResciptNO'),
				'invoice_id'=>$invoice->id,
				'approve_by'=>$this->session->userdata('userId'),
				'pay_by'=>$this->session->userdata('userId'),
				'invoice_no'=>$invoice->invoice_no
			];
			$this->basic_operation_m->insert('tbl_domestic_topay_invoice_bill', $data);
			$this->db->update('tbl_domestic_invoice',['billing_status'=>1],['id'=>$invoice->id]);
			redirect('admin/view-topay-billing');
		 }
	}
}
