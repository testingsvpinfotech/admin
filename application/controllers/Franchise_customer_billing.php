...
use Dompdf\Dompdf;
class Franchise_customer_billing extends CI_Controller {
	...
	public function index() 
	{
		...
		if (isset($_POST['submit'])) 
		{
			...
			if($_SESSION['userType'] == 1 || $_SESSION['userType'] ==10)
			{
				...
				'booking_date <='=>$toDate,
				'dispatch_details' => $this->input->post('bill_type')
				);
		...
		if (isset($_POST['submit_print'])) 
		{
			...
			if(!empty($max_number) && !empty($max_number->id )){$inc_num=(($max_number->id )+1); }else{$inc_num=52;}
			...
			$this->db->trans_start();
			$this->db->insert('tbl_franchise_invoice', $invoice);
			$invoice_id = $this->db->insert_id();

			$selected_dockets = $this->input->post('selected_dockets');

			$whr1 = array('tbl_domestic_booking.customer_id'=>$customer_id);
			
			$data['allpoddata']= $this->booking_model->get_all_pod_data_domestic($whr1,$selected_dockets);
			...
			$fuel_charges = 0;
				...
				//  getting branch gst number 
			foreach ($data['allpoddata'] as $pod)
			{
				$whr_c = array('id'=>$pod['reciever_city']) ;
				...
				$this->basic_operation_m->update('tbl_domestic_booking',$d_update,$d_whr);
				...
				$invoice_detail['pickup_charges']   	= $pod['pickup_charges'];
				$invoice_detail['delivery_charges'] 	= $pod['delivery_charges'];
				$invoice_detail['courier_charges']  	= $pod['courier_charges'];
				$invoice_detail['awb_charges']      	= $pod['awb_charges'];
				...
				$this->db->insert('tbl_franchise_invoice_detail', $invoice_detail);
				...
				$customer_details = $this->basic_operation_m->get_table_row('tbl_customers',['customer_id'=>$customer_id]);
				$customer_gst = substr(trim($customer_details->gstno),0, 2);
				if($branch_gst==$customer_gst)
				{
                    // calculation about gst 
					$cgst = ($sub_total/100)*$gstCharges->cgst;
					$sgst = ($sub_total/100)*$gstCharges->sgst;

					$grand_total1 = $sub_total + $cgst + $sgst;
					
					$invoiceData['cgst_per'] 		= $gstCharges->cgst;
					$invoiceData['sgst_per'] 		= $gstCharges->sgst;
					$invoiceData['igst_per'] 		= '0.0';
					$invoiceData['cgst_amount'] 	= $cgst;
					$invoiceData['sgst_amount'] 	= $sgst;			                
					$invoiceData['igst_amount'] 	= '0.0';	
					$invoiceData['grand_total']		= $grand_total1;
				}else{
					$invoiceData['cgst_per'] 		= '0.0';
					$invoiceData['sgst_per'] 		= '0.0';
					$invoiceData['igst_per'] 		= $gstCharges->igst;
					$invoiceData['cgst_amount'] 	= '0.0';
					$invoiceData['sgst_amount'] 	= '0.0';
					$igst = ($sub_total/100)*$gstCharges->igst;		                
					$grand_total1 = $sub_total + $igst;	
					$invoiceData['igst_amount'] 	= $igst;
					$invoiceData['grand_total']		= $grand_total1;
				}
				// print_r($invoiceData);
				// die;
			    ...
			    $this->db->update('tbl_franchise_invoice', $invoiceData);
			    unset($invoiceData);
			...
			redirect("admin/franchise-domestic-invoice");
		...
		if($_SESSION['userType']==1 ||$_SESSION['userType']==10)
		{
			...
			$data['customers']  = $this->basic_operation_m->get_query_result_array("SELECT * FROM tbl_customers WHERE branch_id = '$branch_id' AND customer_type ='1' AND customer_type ='2' ORDER BY customer_name ASC");
		}
		...
		$data['branches']  = $this->basic_operation_m->get_query_result_array('SELECT * FROM tbl_branch WHERE 1 ORDER BY branch_name ASC');
		$this->load->view('admin/franchise_billing/booking_invoice', $data);
	}

	public function getCustomer()
	{
		$branch_id = $this->input->post('branch_id');
		$whr = array('branch_id' => $branch_id,'customer_type'=>'1','customer_type'=>'2');
	...
	public function save() 
	{
		...
		$tototal_amount = $total + $sgst + $cgst;
		$invoice['amount'] = $tototal_amount;
	...
	{
    	$data['id'] = $id;
    	$data['customer'] = $this->basic_operation_m->get_table_row('tbl_franchise_invoice',array('id'=>$id));
    	$data['allpoddata'] =$this->basic_operation_m->get_all_result('tbl_franchise_invoice_detail',array('invoice_id'=>$id));
    	$data['gstCharges'] =$this->basic_operation_m->get_query_row('select * from tbl_gst_setting order by id desc limit 1');
		
    	$this->load->view('admin/franchise_billing/edit_invoice', $data);
    }

    public function cash_invoice_edit($id, $customer_id){
    	$data['id'] = $id;
    	$data['customer'] = $this->basic_operation_m->get_table_row('tbl_domestic_invoice',array('id'=>$id));
    	$data['allpoddata'] =$this->basic_operation_m->get_all_result('tbl_domestic_invoice_detail',array('invoice_id'=>$id));
    	$data['gstCharges'] =$this->basic_operation_m->get_query_row('select * from tbl_gst_setting order by id desc limit 1');
		
		// echo "<pre>"; print_r($data); die;

    	$this->load->view('admin/franchise_billing/edit_cash_invoice', $data);
    }
    
	public function final_invoice_edit($id) 
	{
    	...
    	$this->load->view('admin/franchise_billing/edit_final_invoice', $data);
    }
	
	public function invoice_view($id,$company_id) {
    	$data['id'] = $id;
    	$in = $this->db->query("select * from tbl_franchise_invoice where id = " . $id."");
    	$data['customer'] = $in->row();
    	$in_de = $this->db->query("select * from tbl_franchise_invoice_detail where invoice_id = " . $id);
    	...
    	$this->load->view('admin/franchise_billing/billing_invoice_view', $data);
    }

    public function cash_invoice_view($id, $company_id){
    	$data['id'] = $id;
    	$in = $this->db->query("select * from tbl_domestic_invoice where id = " . $id." ORDER BY inc_num DESC  ");
    	$data['customer'] = $in->row();
    	$in_de = $this->db->query("select * from tbl_domestic_invoice_detail where invoice_id = " . $id);
    	$data['allpoddata'] = $in_de->result_array();
    	...
    	$where =array('id'=>$company_id);
		$data['company_details'] = $this->basic_operation_m->get_table_row('tbl_company',$where);
		$data['branch'] = $this->db->query('SELECT inv.* FROM tbl_domestic_invoice inv 
			-- LEFT JOIN tbl_domestic_booking b ON(b.booking_id = inv.booking_id)
			-- LEFT JOIN tbl_branch b1 ON(b.branch_id = b1.branch_id)
			WHERE inv.invoice_id ='.$id);
		// echo "<pre>"; print_r($data); die;

    	$this->load->view('admin/franchise_billing/billing_cash_invoice_view', $data);
    }
	...
	public function get_invoiceNumber($invoicePrefix){
		...
		$in = $this->db->query("select invoice_prefix from tbl_domestic_invoice where invoice_prefix = '$invoicePrefix'");
...
ini_set('display_errors', 1);
		...
		$year = date('y');
		$invoicePrefix = $branch_info->domestic_invoice_series.'/'.$year.'-'.($year+1);
		$invoice['invoice_prefix'] = $invoicePrefix;
		
		$invoice['cgst_amount'] = $invoice['cgst'];
		$invoice['sgst_amount'] = $invoice['sgst'];
		$invoice['igst_amount'] = $invoice['igst'];
    	...
    	$this->db->where('id', $id);
    	$this->db->update('tbl_franchise_invoice', $invoice);
		$list = $this->input->post('list');
        ...
        $query = "select tbl_franchise_invoice.*, tbl_customers.pincode, tbl_customers.phone, tbl_customers.sac_code from tbl_franchise_invoice LEFT JOIN tbl_customers ON tbl_franchise_invoice .cid = tbl_customers.cid where tbl_franchise_invoice.id =".$id;
        
        $data['customer'] = $this->basic_operation_m->get_query_row($query);
        $inc_num 	=$data['customer']->inc_num;
		$company_id =$data['customer']->company_id;
       
        $in_de = "select tbl_franchise_invoice_detail.*, tbl_domestic_booking.invoice_no as booking_invoice_num, tbl_domestic_booking.special_instruction from tbl_franchise_invoice_detail INNER JOIN tbl_domestic_booking ON tbl_franchise_invoice_detail.booking_id = tbl_domestic_booking.booking_id where tbl_franchise_invoice_detail.invoice_id =".$id." ORDER BY tbl_domestic_booking.booking_date ASC";
		...
		$data['total'] 					= $data['customer']->total_amount;
		$data['amountt'] 				= $data['customer']->amount;
		$data['branch_info'] 			= $branch_info;
		
		$where =array('id'=>$company_id);
		$data['company_details'] = $this->basic_operation_m->get_table_row('tbl_company',$where);
		$invoice_series = $data['company_details']->domestic_invoice_series;
		
		
		$html = $this->load->view('admin/franchise_billing/booking_print', $data, true);	
		// echo $html; die;
		...
		{
			redirect('admin/list-franchise-finalInvoice'); 
		}
		else
		{
			redirect('admin/list-domestic-billing'); 
		}
	}

	public function save_cash_edit($id) 
	{
		...
		$in = $this->db->query("select * from tbl_domestic_invoice where id = '$id'");
        $invoiceData = $in->row_array();
		$data['invoiceData'] = $invoiceData;
		...
		if($invoice['igst_per']	 > 0)
		{
			$invoice['igst_amount'] = (($invoice['sub_total'] / 100) * $invoice['igst_per']);
			$invoice['cgst_amount'] = '0';
			$invoice['sgst_amount'] = '0';
		}
		elseif($invoice['cgst_per']	 > 0 && $invoice['sgst_per'] > 0)
		{
			$invoice['cgst_amount'] = (($invoice['sub_total'] / 100) * $invoice['cgst_per']);
			$invoice['sgst_amount'] = (($invoice['sub_total'] / 100) * $invoice['sgst_per']);
    	...
    	$invoice['amount'] 				= $invoice['sub_total'] + $invoice['igst_amount']  + $invoice['sgst_amount'] + $invoice['cgst_amount'];
    	$invoice['grand_total'] 		= $invoice['sub_total'] + $invoice['igst_amount']  + $invoice['sgst_amount'] + $invoice['cgst_amount'];
    	
    	
    	$invoice['total_amount'] 		= $this->input->post('sub_total');
    	$invoice['final_invoice'] 		= $this->input->post('final_invoice') ? 1 : 0;
        ...
        unset($invoice['total']);
		$invoice['approveId'] = ($invoice['approve_status'] == 1)?$this->session->userdata('userId'):0;
		$invoice['approveDtm'] = date('Y-m-d H:i:s');

    	$this->db->where('id', $id);
    	$this->db->update('tbl_domestic_invoice', $invoice);
		$list = $this->input->post('list');
        ...
        $query = "select tbl_domestic_invoice.*, tbl_customers.pincode, tbl_customers.phone, tbl_customers.sac_code from tbl_domestic_invoice LEFT JOIN tbl_customers ON tbl_domestic_invoice .cid = tbl_customers.cid where tbl_domestic_invoice.id =".$id;
        
        $data['customer'] = $this->basic_operation_m->get_query_row($query);
        $inc_num 	=$data['customer']->inc_num;
		$company_id =$data['customer']->company_id;
       
        $in_de = "select tbl_domestic_invoice_detail.*, tbl_domestic_booking.invoice_no as booking_invoice_num, tbl_domestic_booking.special_instruction from tbl_domestic_invoice_detail INNER JOIN tbl_domestic_booking ON tbl_domestic_invoice_detail.booking_id = tbl_domestic_booking.booking_id where tbl_domestic_invoice_detail.invoice_id =".$id." ORDER BY tbl_domestic_booking.booking_date ASC";
        ...
        $this->m_pdf->pdf->Output($savefolderpath.$filename, $type);
			...
			redirect('admin/list-domestic-cash-invoice');
	}
    ...
    public function invoice() {    	
    		...
    		$this->load->helper('form_helper');
    		if ($this->session->userdata("userType") == '1') {
    			$resAct = $this->db->query("select * from tbl_franchise_invoice where final_invoice = 0 and (payment_type != 'Cash' or payment_type != 'CASH' or payment_type != 'ToPay' or payment_type != 'TOPAY') ORDER BY tbl_franchise_invoice.id desc");
    			...
    			$resAct = $this->db->query("select * from tbl_franchise_invoice where branch_id='$branch_id' and final_invoice = 0 and (payment_type != 'Cash' or payment_type != 'CASH' or payment_type != 'ToPay' or payment_type != 'TOPAY') ORDER BY tbl_franchise_invoice.id desc");
    			if ($resAct->num_rows() > 0) {
    				$data['allpoddata'] = $resAct->result_array();
    			}
    		}

    		$this->load->view('admin/franchise_billing/view_invoice', $data);
    ...
    public function cash_invoice(){
		...
		$this->load->helper('form_helper');
		if ($this->session->userdata("userType") == '1') {
			$resAct = $this->db->query("SELECT i.*,b.branch_name, u.full_name as created_by, u1.full_name as approved_by  FROM tbl_domestic_invoice i 
				LEFT JOIN tbl_branch b ON(b.branch_id = i.branch_id) 
				LEFT JOIN tbl_users u ON(u.user_id = i.createId) 
				LEFT JOIN tbl_users u1 ON(u1.user_id = i.approveId) 
				WHERE (i.payment_type = 'Cash' OR i.payment_type = 'CASH') ORDER BY i.id DESC");
			if ($resAct->num_rows() > 0) {
				$data['allpoddata'] = $resAct->result_array();
			}
		} else {
			...
			$resAct = $this->db->query("SELECT i.*,b.branch_name, u.full_name as created_by FROM tbl_domestic_invoice i LEFT JOIN tbl_branch b ON(b.branch_id = i.branch_id) LEFT JOIN tbl_users u ON(u.user_id = i.createId) WHERE i.branch_id='$branch_id' AND i.payment_type = 'Cash' ORDER BY i.id dESC");
			if ($resAct->num_rows() > 0) {
				$data['allpoddata'] = $resAct->result_array();
			}
		}
		...
		$this->load->view('admin/franchise_billing/view_cash_invoice', $data);
	}
    ...
    public function finalInvoice() {
    		...
    		$this->load->helper('form_helper');
    		if ($this->session->userdata("userType") == '1') {
    			// (payment_type != 'Cash' or payment_type != 'CASH' or payment_type != 'ToPay' or payment_type != 'TOPAY')
    			$whrCon = array('final_invoice' => '1','payment_type !=' => 'TOPAY','payment_type !=' => 'CASH');
    			$data['allpoddata'] = $this->booking_model->select_domestic_invoice_details1($whrCon);
				// echo $this->db->last_query();die; 	
    		} 
			else 
    		{
				...
				$branch_id = $res->branch_id;
				$whrCon = array('branch_id' => $branch_id,'final_invoice'=>'1', 'payment_type !=' => 'TOPAY', 'payment_type !=' => 'CASH');
    			$data['allpoddata'] = $this->booking_model->select_domestic_invoice_details1($whrCon);    			
    		} 
			$where =array('id'=>1);
		    $data['company_details'] = $this->basic_operation_m->get_table_row('tbl_company',$where);

	        $data['prefix'] = $data['company_details']->domestic_invoice_series;	        
    		$this->load->view('admin/franchise_billing/view_invoice_final_list', $data);
	...
	public function send_mail($invoice_id, $customer_id) 
	{
	  ...
	  //  $email="itsprachee08@gmail.com";
	    ...
	    $inv_details = $this->db->query("select invoice_number,inc_num,company_id from  tbl_domestic_invoice WHERE id='$invoice_id'");
        ...
        $where =array('id'=>$company_id);
		$data['company_details'] = $this->basic_operation_m->get_table_row('tbl_company',$where);
	    $prefix = $data['company_details']->domestic_invoice_series;
...
//echo $message;
      ...
      $this->email->attach(base_url('assets/invoice/domestic/' . $name));
     if($this->email->send())
     {
         $data=array('send_mail_status'=>1);
         $whr =array('id'=>$invoice_id,'customer_id'=>$customer_id);
         $this->basic_operation_m->update('tbl_domestic_invoice',$data,$whr);
     }
      redirect('admin/list-domestic-finalInvoice'); 
}
    ...
    public function invoice_delete() 
	{
	    $id = $this->input->post('getid');
	   	if(!empty($id)){
      	$this->db->where('id', $id);
      	$this->db->delete('tbl_domestic_invoice');
      	
      	$whr =array('invoice_id'=> $id);
      	$booking_id_details = $this->basic_operation_m->get_all_result("tbl_domestic_invoice_detail",$whr);
        ...
        $this->db->where('invoice_id', $id);
      	$this->db->delete('tbl_domestic_invoice_detail');
      	...
      	$output['status'] = 'success';
			...
			$output['status'] = 'error';
			$output['message'] = 'Something went wrong in deleting the Invoice';
		}
...
//         $this->db->where('invoice_id', $id);
//       	$this->db->delete('tbl_domestic_invoice_detail');
//       	redirect('admin/list-domestic-booking');
//     }
	
public function invoice_detail_delete($booking_id) 
{
	$d_update =array('invoice_generated_status'=>'0');
	$d_whr=array('booking_id'=>$booking_id);
	$this->basic_operation_m->update('tbl_domestic_booking',$d_update,$d_whr);
	$get = $this->db->query("select tbl_domestic_invoice.id as id , tbl_domestic_invoice.customer_id as customer_id from tbl_domestic_invoice_detail JOIN tbl_domestic_invoice on tbl_domestic_invoice.id = tbl_domestic_invoice_detail.invoice_id where tbl_domestic_invoice_detail.booking_id ='$booking_id'")->row();
	$this->basic_operation_m->delete('tbl_domestic_invoice_detail',array('booking_id'=>$booking_id));
	  redirect('admin/show-edit-domestic-invoice/'.$get->id.'/'.$get->customer_id);
}
     

	public function domestic_excel($invoiceId,$company_id)
	{
		// load excel library 
		...
		$invoice_data 		= $this->Invoice_model->getDomesticInvoice($invoiceId);
		$invoiceDeatil_data = $this->Invoice_model->getDomesticInvoiceDetail($invoiceId);
   ...
   public function payments(){	
		...
		$this->load->helper('form_helper');
		if ($this->session->userdata("userType") == '1') {
			$resAct = $this->db->query("select * from tbl_domestic_invoice where final_invoice = 1");
			if ($resAct->num_rows() > 0) {
				$data['allpoddata'] = $resAct->result_array();
			}
		} else {
			...
			$resAct = $this->db->query("select * from tbl_domestic_invoice where branch_id='$branch_id' and final_invoice = 1");
			if ($resAct->num_rows() > 0) {
				$data['allpoddata'] = $resAct->result_array();
			}
		}
		...
		$data['prefix'] = $prefix;
		$this->load->view('admin/franchise_billing/view_payments', $data);
...
//                $data = array(); 
   ...
   public function save_payment(){
	   ...
	   $resAct = $this->db->query("select * from tbl_domestic_invoice where id = $invoice_id");
	   ...
	   $this->db->insert('tbl_domestic_invoice_payments', $savedata);
	   ...
	   redirect('admin/list-domestic-payments');
	}

	public function payment_history(){		
		$userid = $this->session->userdata('userId');

		$data['payment_history'] = $this->booking_model->show_domestic_payment_history($userid);
		$this->load->view('admin/franchise_billing/view_payment_history', $data);
	}
	...
	public function get_data(){
		$html = $this->load->view('admin/franchise_billing/booking_print');
	}

	// TOPAY LIST START
	public function topay_invoice(){
		$data = array();
		$this->load->helper('form_helper');
		if ($this->session->userdata("userType") == '1') {
			$resAct = $this->db->query("SELECT i.*,b.branch_name, u.full_name as created_by FROM tbl_domestic_invoice i 
				LEFT JOIN tbl_branch b ON(b.branch_id = i.branch_id) 
				LEFT JOIN tbl_users u ON(u.user_id = i.createId) 
				WHERE (i.payment_type = 'ToPay' OR i.payment_type = 'TOPAY')");
			if ($resAct->num_rows() > 0) {
				$data['allpoddata'] = $resAct->result_array();
			}
		} else {
			...
			$resAct = $this->db->query("SELECT i.*,b.branch_name, u.full_name as created_by FROM tbl_domestic_invoice i LEFT JOIN tbl_branch b ON(b.branch_id = i.branch_id) LEFT JOIN tbl_users u ON(u.user_id = i.createId) WHERE i.branch_id='$branch_id' AND (i.payment_type = 'ToPay' OR i.payment_type = 'TOPAY') ORDER BY i.id dESC");
			if ($resAct->num_rows() > 0) {
				$data['allpoddata'] = $resAct->result_array();
			}
		}
		$data['pay_method'] = $this->db->get('payment_method')->result();
		// echo "<pre>"; print_r($data['allpoddata']); die;
		$this->load->view('admin/franchise_billing/view_topay_invoice', $data);
	}

	public function topay_invoice_edit($id){
		$data['id'] = $id;
    	$data['customer'] = $this->basic_operation_m->get_table_row('tbl_domestic_invoice',array('id'=>$id));
    	$data['allpoddata'] =$this->basic_operation_m->get_all_result('tbl_domestic_invoice_detail',array('invoice_id'=>$id));
    	$data['gstCharges'] =$this->basic_operation_m->get_query_row('select * from tbl_gst_setting order by id desc limit 1');
		
    	$this->load->view('admin/franchise_billing/edit_topay_invoice', $data);		
	}

	public function topay_invoice_view($id){
    	...
    	$data['allpoddata'] = $this->db->query("SELECT * FROM tbl_domestic_invoice_detail WHERE invoice_id = " . $id)->result_array();
    	$pod = $data['allpoddata'][0]['pod_no'];
    	$data['booking'] = $this->db->get_where('tbl_domestic_booking', ['pod_no' => $pod])->row();
		...
		$data['branch'] = $this->basic_operation_m->get_table_row('tbl_branch',['branch_id'=>$data['user']->branch_id]);

		// echo "<pre>"; print_r($data['branch']); die;
    	$this->load->view('admin/franchise_billing/billing_topay_invoice_view', $data);		
	}

	public function update_topay_payable_details($id){
		$data = $this->input->post();
		$pay_by = $this->session->userdata('userId');
		$result = $this->db->update('tbl_domestic_invoice', ['ref_name' =>$data['ref_name'],'pay_by' =>$pay_by,'pay_method'=>$data['pay_method'],'pay_date'=> $data['pay_date']],['id' => $id]);
		echo !empty($result)?1:0;
	}
	public function update_topay_payable_details1($id){
		$data = $this->input->post();
		$pay_by = $this->session->userdata('userId');
		$result = $this->db->update('tbl_domestic_invoice', ['ref_name' =>$data['ref_name1'],'edit_pay_by' =>$pay_by,'pay_method'=>$data['pay_method1'],'edit_pay_date'=> $data['pay_date1']],['id' => $id]);
		echo !empty($result)?1:0;
	}
	// TOPAY LIST END
}
