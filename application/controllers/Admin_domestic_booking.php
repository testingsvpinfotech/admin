...
use Dompdf\Dompdf;
class Admin_domestic_booking extends CI_Controller {
	...
	public function index() 
	{
		...
		if (isset($_POST['submit_print'])) 
		{
			...
			foreach ($data['allpoddata'] as $pod)
			{
				...
				// print_r($branch_gst).'<br>';
				// print_r($customer_gst);die;
				...
				$customer_details = $this->basic_operation_m->get_table_row('tbl_customers',['customer_id'=>$customer_id]);
				$customer_gst = substr(trim($customer_details->gstno),0, 2);
				//$gst = $this->basic_operation_m->get_table_row('tbl_gst_setting',['id'=>1]);
                //  print_r($total_sgst).'<br>';
			    ...
			    $this->db->update('tbl_domestic_invoice', $invoiceData);
			    unset($invoiceData);
			...
			redirect("admin/list-domestic-invoice");
		...
		$this->load->view('admin/booking_domestic_master/booking_invoice', $data);
    ...
    public function invoice_edit($id,$customer_id) 
	{
    	...
    	$this->load->view('admin/booking_domestic_master/edit_invoice', $data);
    }
    ...
    public function cash_invoice_edit($id, $customer_id){
    	...
    	$this->load->view('admin/booking_domestic_master/edit_cash_invoice', $data);
    }
	...
	public function final_invoice_edit($id) 
	{
    	...
    	$this->load->view('admin/booking_domestic_master/edit_final_invoice', $data);
    }
	...
	public function invoice_view($id,$company_id) {
    	...
    	$this->load->view('admin/booking_domestic_master/billing_invoice_view', $data);
    }
    ...
    public function cash_invoice_view($id, $company_id){
    	...
    	$this->load->view('admin/booking_domestic_master/billing_cash_invoice_view', $data);
    }
    ...
    public function invoice() {    	
    		...
    		$this->load->helper('form_helper');
    		if ($this->session->userdata("userType") == '1') {
    			$resAct = $this->db->query("select * from tbl_domestic_invoice where final_invoice = 0 and (payment_type != 'Cash' or payment_type != 'CASH' or payment_type != 'ToPay' or payment_type != 'TOPAY') ORDER BY tbl_domestic_invoice.id desc");
    			...
    			$resAct = $this->db->query("select * from tbl_domestic_invoice where branch_id='$branch_id' and final_invoice = 0 and (payment_type != 'Cash' or payment_type != 'CASH' or payment_type != 'ToPay' or payment_type != 'TOPAY') ORDER BY tbl_domestic_invoice.id desc");
    			if ($resAct->num_rows() > 0) {
    				$data['allpoddata'] = $resAct->result_array();
    			}
    		}

    		$this->load->view('admin/booking_domestic_master/view_invoice', $data);
    ...
    public function cash_invoice(){
		...
		$this->load->view('admin/booking_domestic_master/view_cash_invoice', $data);
	}
    ...
    public function finalInvoice() {
    		...
    		if ($this->session->userdata("userType") == '1') {
    			...
    			$data['allpoddata'] = $this->booking_model->select_domestic_invoice_details($whrCon);
				// echo $this->db->last_query();die; 	
    		} 
			else 
    		{
			...
			// echo $this->db->last_query();die; 		
	        // $invoiceSettings = $this->booking_model->select_invoice_setting_details($whrCon);
    		...
    		$this->load->view('admin/booking_domestic_master/view_invoice_final_list', $data);
...
public function invoice_detail_delete($booking_id) 
{
   ...
   public function payments(){	
		...
		unset($resInoviceSetting);
		$data['prefix'] = $prefix;
		$this->load->view('admin/booking_domestic_master/view_payments', $data);
...
//                $data = array(); 
   ...
   public function save_payment(){
	...
	public function payment_history(){		
		...
		$this->load->view('admin/booking_domestic_master/view_payment_history', $data);
	}
	...
	public function get_data(){
		$html = $this->load->view('admin/booking_domestic_master/booking_print');
	}
	...
	public function topay_invoice(){
		...
		$this->load->view('admin/booking_domestic_master/view_topay_invoice', $data);
	}
	...
	public function topay_invoice_edit($id){
    	...
    	$this->load->view('admin/booking_domestic_master/edit_topay_invoice', $data);		
	}
	...
	public function topay_invoice_view($id){
    	...
    	$this->load->view('admin/booking_domestic_master/billing_topay_invoice_view', $data);		
	}
